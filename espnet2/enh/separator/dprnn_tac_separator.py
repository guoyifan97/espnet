...
from espnet2.enh.layers.tac_utility.models import *
...
class DPRNNTACSeparator(AbsSeparator):
    def __init__(
        self, 
        ...
        # DPRNN-TAC
        self.separator = BF_module(self.enc_dim+(self.context*2+1)**2, 
                                   self.feature_dim, self.hidden_dim, 
                                   self.enc_dim, self._num_spk, self.layer, 
                                   self.segment_size, model_type='DPRNN_TAC')
    ...
    def forward(self, input, flens, num_mic=None):
        # B T C -> B C T
        ...
        enc_context_copy = enc_context.permute(0,4,1,3,2).contiguous().view(batch_size*seq_length, nmic, 
                                                                            -1, self.enc_dim)  # B*L, nmic, 2C+1, N
        ...
        norm_context = torch.cat([norm_context]*self._num_spk, 1)  # B, nspk, N, 2C+1, L
        ...
        all_filter = all_filter.view(batch_size, self._num_spk, seq_length, self.enc_dim, -1)  # B, nspk, L, N+1, 2C+1
        ...
        # decode
        bf_signal = self.decoder(output.view(batch_size*self._num_spk, self.enc_dim, -1))  # B*nspk, 1, T
        
        if rest > 0:
            bf_signal = bf_signal[:,:,self.stride:-rest-self.stride]

        bf_signal = bf_signal.view(batch_size, self._num_spk, -1)  # B, nspk, T

        bf_signal = torch.unbind(bf_signal, dim=1) # [B, T]*num_spk

        return bf_signal, flens, None
...
if __name__ == "__main__":
    model_iFaSNet = iFaSNet(enc_dim=64, feature_dim=64, hidden_dim=128, layer=6, segment_size=24, 
                            num_spk=2, win_len=16, context_len=16, sr=16000)
