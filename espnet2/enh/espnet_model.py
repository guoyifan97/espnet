...
class ESPnetEnhancementModel(AbsESPnetModel):
    ...
    def _compute_loss(
        ...
        """
        # (Batch, samples) or (Batch, samples, channels) -> B T N or B T N Channels
        feature_mix, flens = self.encoder(speech_mix, speech_lengths)
        # B T N or B T N C -> B T N
    ...
    def _permutation_loss(ref, inf, criterion, perm=None):
        ...
        """
        # ref: (Batch, num_speaker, samples) or (Batch, num_speaker, samples, channels)
        # inf: num_spk, B, T

        # B, num_spk, T -> num_spk, B, T
        # ref = ref.transpose(0, 1)
        assert len(ref) == len(inf), (len(ref), len(inf), ref[0].shape, inf[0].shape)
        num_spk = len(ref)
