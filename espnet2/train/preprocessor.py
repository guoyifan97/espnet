...
class CommonPreprocessorMultiSpkrASR(AbsPreprocessor):
    def __init__(
        self,
        train: bool,
        token_type: str = None,
    ...
    def __call__(
        ...
        if isinstance(self.text_name, str) and self.text_name in data and self.tokenizer is not None:
            text = data[self.text_name]
            text = self.text_cleaner(text)
            tokens = self.tokenizer.text2tokens(text)
            text_ints = self.token_id_converter.tokens2ids(tokens)
            data[self.text_name] = np.array(text_ints, dtype=np.int64)
        elif isinstance(self.text_name, list):
            for text_name in self.text_name:
                if text_name in data and self.tokenizer is not None:
                    text = data[text_name]
                    text = self.text_cleaner(text)
                    tokens = self.tokenizer.text2tokens(text)
                    text_ints = self.token_id_converter.tokens2ids(tokens)
