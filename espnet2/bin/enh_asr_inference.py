...
def utterance_normalize(wav):
    wav = wav / max(abs(wav)) * 0.88
    return wav
...
class Speech2Text:
    ...
    def __call__(
        ...
        if self.joint_model.enh_subclass != None:
            ...
            if self.joint_model.cal_enh_loss:
                if speech_refs[0].dim() == 2:
                    speech_refs = [speech_ref[:, 0] for speech_ref in speech_refs]
                ref = np.array(torch.stack(speech_refs, dim=0).squeeze())  # nspk,T
                inf = np.array(torch.stack(speech_pre, dim=1).squeeze().cpu()) # nspk,T
            ...
            if self.enhance_wav:
                results_list = [np.array([inf[p, :] for p in perm]), ref]
            else:
            # b. Forward Encoder
                results_list = []
            ...
            for idx, p in enumerate(perm):
                ...
                # debug
                if self.debug:
                    speech_spk = speech_refs[idx].unsqueeze(0)
                    # enc, _ = self.joint_model.encode(speech_spk, batch['speech_lengths'])
                    enc, _ = self.joint_model.asr_subclass.encode(
                        speech_spk, speech_pre_lengths
                    )
            ...
            results_list = []
            encs = self.joint_model.asr_subclass.encode(batch["speech"], batch["speech_lengths"])[0]
            sdr = None
            # For each predicted spk
            for idx, p in enumerate(range(self.joint_model.num_spk)):
                # speech_spk = speech_pre[int(p)]
                # # enc, _ = self.joint_model.encode(speech_spk, batch['speech_lengths'])
                # enc, _ = self.joint_model.asr_subclass.encode(
                #     speech_spk, speech_pre_lengths
                # )
                enc = encs[p].unsqueeze(0) # numspk*B T F ->  T F -> 1 T F
                assert len(enc) == 1, len(enc)
...
def inference(
    ...
    with DatadirWriter(output_dir) as writer:
        for keys, batch in loader:
            ...
            speech_refs = [v for k, v in batch.items() if k.startswith("speech_ref")]

            # N-best list of (text, token, token_int, hyp_object)
            logging.info(f"keys: {keys}")
            results_list = speech2text(batch["speech_mix"], speech_refs)
            if enhance_wav:
                # 这块是之前额外添加了两项，pop()不影响解码结果
                speech_pre = results_list.pop(0)
                speech_ref = results_list.pop(0)
                if normalize_output_wav:
                    speech_pre = [utterance_normalize(speech_pre[0]), utterance_normalize(speech_pre[1])]
                    speech_ref = [utterance_normalize(speech_ref[0]), utterance_normalize(speech_ref[1])]
                else:
                    speech_pre = [speech_pre[0], speech_pre[1]]
                    speech_ref = [speech_ref[0], speech_ref[1]]
                sf.write(f"{enhance_exp}/{keys}_mix.wav", batch["speech_mix"].cpu().numpy(), fs)
                sf.write(f"{enhance_exp}/{keys}_pre_spk1.wav", speech_pre[0], fs)
                sf.write(f"{enhance_exp}/{keys}_pre_spk2.wav", speech_pre[1], fs)
                sf.write(f"{enhance_exp}/{keys}_ref_spk1.wav", speech_ref[0], fs)
                sf.write(f"{enhance_exp}/{keys}_ref_spk2.wav", speech_ref[1], fs)
...
def get_parser():
    ...
    group.add_argument(
        ...
        type=str2bool,
        default=False,
        help="Whether to enhance wav to [-1~1]",
    )
    group.add_argument(
        "--enhance_exp",
        type=str_or_none,
        default=None,
        help="The exp path of the enhanced_wav. "
        "If not given, refers from the training args",
    )
    group.add_argument(
        "--debug",
