...
class ESPnetEnhASRModel(AbsESPnetModel):
    ...
    def __init__(
        ...
        self.cal_frontend_loss = cal_frontend_loss

        # GUO
        if self.enh_subclass == None:
            self.use_frontend_as_sep = True
        else:
            self.use_frontend_as_sep = False

        # TODO(Jing): find out the -1 or 0 here
        # self.idx_blank = token_list.index(sym_blank) # 0
        self.idx_blank = -1
        self.num_spk = enh_model.num_spk if enh_model != None else asr_model.frontend.frontend.beamformer.mimo_num_spk
        if self.num_spk > 1:
            assert (
        ...
        if not self.end2end_train: # GUO
            assert self.enh_subclass != None
        self.enh_attr = dir(enh_model) if not self.use_frontend_as_sep else dir(ESPnetEnhancementModel)
        ...
        if not self.use_frontend_as_sep:
            ...
            self.asr_attr = ['_calc_att_loss', '_calc_ctc_loss', '_calc_rnnt_loss', '_extract_feats', 'criterion_att', 'ctc', 'ctc_weight', 'encode', 'eos', 'error_calculator', 'extract_feats_in_collect_stats', 'frontend', 'ignore_id', 'normalize', 'postencoder', 'preencoder', 'rnnt_decoder', 'sos', 'specaug', 'token_list', 'vocab_size']
            for arr in self.asr_attr:
                setattr(self, arr, getattr(self.asr_subclass, arr))
    ...
    def forward(
        ...
        **kwargs,
        ...
        if utt2category == UttCategory.SIMU_DATA or (
            ...
            if self.use_frontend_as_sep: # GUO
                # Using the frontend as the seperator
                # B T C
                speech_pre_all = speech_mix
                speech_pre_lengths = speech_mix_lengths
                # text_ref_all, text_ref_lengths = text_ref[0], text_ref_lengths[0]

                # [(B, T)*num_spk] -> num_spk*B T
                text_ref_all = torch.cat(text_ref, dim=0)
                text_ref_lengths = torch.cat(text_ref_lengths, dim=0)

                loss_enh, perm = None, None
                n_speaker_asr = self.num_spk
            elif is_simu_data or random.random() <= self.enh_real_prob:
                ...
                loss_enh, perm, speech_pre, speech_pre_lengths = self.forward_enh(
                    speech_mix,
                    speech_mix_lengths,
                    cal_loss=(is_simu_data and self.cal_enh_loss), # GUO
                    **kwargs,
                )
                ...
                if speech_mix.dim() == 2:  # single-channel input or use frontend to sep GUO
                ...
                if is_simu_data and not self.end2end_train:
                    ...
                    # multichannel case
                    if speech_pre.dim() == 4:
                        speech_pre = speech_pre[..., random.randint(0, speech_pre.size(-1)-1)].squeeze(dim=-1)
                ...
                speech_pre_lengths = torch.cat(
                    [speech_mix_lengths for _ in range(self.num_spk)], dim=0
                )
        ...
        # # numspk*batch T F
        # with autocast(False):
        #     out = self._extract_feats(speech_pre_all, speech_pre_lengths)
        #     if len(out) == 2:
        #         # numspk*batch T F
        #         feats, feats_lengths = out
        #         loss_frontend = None
        #     elif len(out) == 3:
        #         feats, feats_lengths, loss_frontend = out
        #     else:
        #         raise

        # use frontend as seperator and use enh loss to help training
        if self.use_frontend_as_sep and self.cal_enh_loss and utt2category == UttCategory.SIMU_DATA:
            encoder_out, encoder_out_lens, loss_enh, perm, loss_frontend = self.encode_with_calc_enh_loss(
                speech_pre_all, 
                speech_pre_lengths,
                **kwargs,
            )
            ...
            out = self.encode(speech_pre_all, speech_pre_lengths)
            if len(out) == 2:
                encoder_out, encoder_out_lens = out
                loss_frontend = None
            else:
                encoder_out, encoder_out_lens, loss_frontend = out
                loss_frontend = loss_frontend
        ...
        if self.cal_frontend_loss and loss_frontend != None:
            loss_asr = loss_asr + loss_frontend
        else:
            loss_frontend = None
        ...
        if self.enh_weight == 0.0 or not self.cal_enh_loss or loss_enh is None:
            ...
            # loss = self.total_loss_scale * (
                # (1 - self.enh_weight) * loss_asr + self.enh_weight * loss_enh * abs(float(loss_asr) / float(loss_enh))
            # )
        ...
        stats = dict(
            ...
            loss_abs_freq_pred=loss_frontend.detach() if loss_frontend is not None else None,
            acc=acc_att,
            cer=cer_att,
            wer=wer_att,
            cer_ctc=cer_ctc,
    ...
    def encode_with_calc_enh_loss(
        self, speech_mix: torch.Tensor, speech_mix_lengths: torch.Tensor, **kwargs,
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, Tuple[torch.Tensor, torch.Tensor], torch.Tensor]:
        """encode the wav features with seperate loss calculation
        ...
        [Optional Args]:
            ...
            ...
        Returns:
            encoder_out, encoder_out_lens, loss_enh, perm, loss_frontend
            encoder_out: torch.Tensor(num_spk*Batch, samples, feature_dim)
            encoder_out_lens: (Batch,)
            loss_enh: scalar
            perm: (Batch,)
            loss_frontend: scalar
        """
        # for data-parallel
        speech_mix = speech_mix[:, : speech_mix_lengths.max()]
        # B T C -> B T C F
        input_stft, feats_lengths = self.frontend._compute_stft(speech_mix, speech_mix_lengths)
        # B T C F -> num_spk*B T F
        input_stft, feats_lengths, loss_frontend = self.frontend.frontend(input_stft, feats_lengths)
        
        input_power = input_stft.real ** 2 + input_stft.imag ** 2

        feats, _ = self.frontend.logmel(input_power, feats_lengths)

        loss_enh, perm = self._calc_enh_loss_with_encoder_out(
            input_stft, 
            torch.cat([speech_mix_lengths for _ in range(self.num_spk)], dim=0), 
            **kwargs
        )

        # resort the prediction wav with the perm from enh_loss
        feats = torch.cat(
            ESPnetEnhancementModel.sort_by_perm(
                torch.chunk(feats, self.num_spk, dim=0),
                perm
            ),
            dim=0
        )
        ...
        if not self.end2end_train:
            # num_spk*B T or num_spk*B T C
            speech_pre = torch.cat(
                [
                    kwargs["speech_ref{}".format(spk + 1)]
                    if f"speech_ref{spk + 1}" in kwargs
                    else None
            ...
            # multichannel case
            if speech_pre.dim() == 3:
                # num_spk*B T C -> num_spk*B T 1 -> num_spk*B T
                speech_pre = speech_pre[..., random.randint(0, speech_pre.size(-1)-1)].squeeze(dim=-1)
            speech_pre_lengths = torch.cat(
                [speech_mix_lengths for _ in range(self.num_spk)], dim=0
            )
            with autocast(False):
                feats, feats_lengths = self._extract_feats(speech_pre, speech_pre_lengths)
        
        # 2. Data augmentation
        if self.asr_subclass.specaug is not None and self.training:
            feats, feats_lengths = self.asr_subclass.specaug(feats, feats_lengths)

        # 3. Normalization for feature: e.g. Global-CMVN, Utterance-CMVN
        if self.asr_subclass.normalize is not None:
            feats, feats_lengths = self.asr_subclass.normalize(feats, feats_lengths)

        # Pre-encoder, e.g. used for raw input data
        if self.asr_subclass.preencoder is not None:
            feats, feats_lengths = self.asr_subclass.preencoder(feats, feats_lengths)

        encoder_out, encoder_out_lens, _ = self.asr_subclass.encoder(feats, feats_lengths)

        if self.asr_subclass.postencoder is not None:
            encoder_out, encoder_out_lens = self.asr_subclass.postencoder(
                encoder_out, encoder_out_lens
            )

        return encoder_out, encoder_out_lens, loss_enh, perm, loss_frontend
        

    def _calc_enh_loss_with_encoder_out(
        self,
        feats: ComplexTensor,
        ...
        **kwargs,
        ...
        if speech_ref[0].dim() == 3:
            speech_ref = [ref[..., 0] for ref in speech_ref]

        # iSTFT: num_spk*B T F -> num_spk*B T
        speech_mix, _ = self.asr_subclass.frontend.stft.inverse(feats, speech_lengths)

        # num_spk*B T -> num_spk * (B, T)
        speech_mix = torch.chunk(speech_mix, self.num_spk, dim=0)

        loss_func = ESPnetEnhancementModel.si_snr_loss_zeromean
        loss, perm = ESPnetEnhancementModel._permutation_loss(speech_ref, speech_mix, loss_func)
    ...
    def _calc_ctc_loss_with_spk(
        ...
        if n_speakers == 1:
            ...
            assert (
                self.ctc.reduce is False
            ), "When solve permutation with CTC, the CTC loss should not be reduced"
            ...
            ys_pad = torch.cat(ys_pad, dim=0) # (n_spk*B, T)
        ...
        for b, b_loss in enumerate(losses_cpu):
            ...
            try:
                ...
                if str(err) == "cost matrix is infeasible":
                    ...
                    logging.info("ctcPIT: cost matrix is infeasible")
                    continue
                else:
                    raise
            
            min_perm_loss.append(torch.mean(losses[b, row_ind, col_ind])) 
            # min_perm_loss.append(torch.sum(losses[b, row_ind, col_ind])) # GUO
            hyp_perm.append(col_ind)
