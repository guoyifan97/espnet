...
if [ $# -ne 3 ]; then
  echo "Arguments should be WHAM script path, WHAM wav path and the WSJ0 path, see local/data.sh for example."
  exit 1;
fi
...
for x in tr cv tt; do
  for ddir in mix_both mix_clean mix_single noise s1 s2; do
    f=${wham_wav_dir}/wav${sample_rate}/${min_or_max}/${x}/${ddir}
    if [ ! -d $f ]; then
...
data=./data
rm -r ${data}/{tr,cv,tt}_mix_{both,clean,single}_${min_or_max}_${sample_rate} 2>/dev/null || true

for x in tr cv tt; do
  for mixtype in both clean single; do
      ddir=${x}_mix_${mixtype}_${min_or_max}_${sample_rate}
      mkdir -p ${data}/${ddir}
      rootdir=${wham_wav_dir}/wav${sample_rate}/${min_or_max}/${x}
      mixwav_dir=${rootdir}/mix_${mixtype}
      awk -v dir="${mixwav_dir}" -F "," \
        'NR>1 {sub(/\.wav$/, "", $1); split($1, lst, "_"); spk=substr(lst[1],1,3)"_"substr(lst[3],1,3); print(spk "_" $1, dir "/" $1 ".wav")}' \
        ${wham_script_dir}/data/mix_2_spk_filenames_${x}.csv | sort > ${data}/${ddir}/wav.scp
...
for mixtype in both clean single; do
  tr=tr_mix_${mixtype}_${min_or_max}_${sample_rate}
  cv=cv_mix_${mixtype}_${min_or_max}_${sample_rate}
  tt=tt_mix_${mixtype}_${min_or_max}_${sample_rate}
