...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        if getattr(args, "use_frontend", False):  # use getattr to keep compatibility
            ...
            if args.use_complex_beamformer:
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        if self.frontend is not None:
            if not self.use_complex_beamformer:
                ...
                hs_pad, hlens, frontend_loss = self.frontend(xs_pad[0], ilens, xs_pad[1], xs_pad[2])
#            hs_pad, hlens = self.feature_transform(hs_pad, hlens)
        ...
        if self.use_complex_beamformer:
            if isinstance(frontend_loss, torch.Tensor):
                # if frontend_loss.device.index==0:
                #     print(f"use frontend loss{frontend_loss}, {self.loss}")
                frontend_loss *= float(self.loss.detach() / frontend_loss.detach())
