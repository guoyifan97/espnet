...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        if getattr(args, "use_frontend", False):  # use getattr to keep compatibility
            ...
            if args.use_complex_beamformer: #use-complex-beamformernew
                self.feature_transform = feature_transform_for_complex(args, (idim-1) * 2)
                # if not getattr(args, "complex_beamformer_new", False):
                self.mask_process = None
                if args.cb_down_sample:
                    self.cb_down_sample = args.cb_down_sample
                    # def mask_process(mask):
                    #     for channel, kernel_size, stride in eval(args.cb_down_sample_layer):
                    #         mask = mask[:, :, :-int(kernel_size-1):stride]
                    #     return mask
                    # self.mask_process = mask_process
        ...
        if getattr(args, "use_complex_beamformer", False):
            self.use_complex_beamformer = True
        else:
            self.use_complex_beamformer = False
        ...
        if getattr(args, "cb_use_frontend_ctc", False):
            self.frontend_ctc = CTC(
                odim, idim, args.dropout_rate, ctc_type=args.ctc_type, reduce=True
            )
...
#            hs_pad, hlens = self.feature_transform(hs_pad, hlens)
            # if hs_pad.device.index == 0:
            #     #print("HS_PAD in e2e_asr_transformer_frontend.py(before transformation):", hs_pad)
            #     print("hs_pad_nan_test:(before transformation)", torch.tensor([torch.isnan(hs_pad.real).any(),torch.isnan(hs_pad.imag).any()]).any())
            #     if torch.tensor((torch.isnan(hs_pad.real).any(), torch.isnan(hs_pad.imag).any())).any():
            #         print("HS_PAD in e2e_asr_transformer_frontend.py(before transformation) WRONG:", hs_pad)
                #print("hs_pad_inf_test:(before transformation)", torch.isinf(hs_pad.real).any(), torch.isinf(hs_pad.imag).any())
            
            # print("1_shape:", hs_pad.shape)
            hs_pad, hlens = self.feature_transform(hs_pad, hlens)
            # print("2_shape:", hs_pad.shape)
            # if hs_pad.device.index == 0:
            #     print("hs_pad_nan_test:(after transformation)", torch.isnan(hs_pad).any())
            #     if torch.isnan(hs_pad).any():
            #         print("HS_PAD in e2e_asr_transformer_frontend.py(after transformation) WRONG:", hs_pad)
        else:
        ...
        # if hs_pad.device.index == 0:
        #     #print("XS_PAD in e2e_asr_transformer_frontend.py:",xs_pad)
        # #print(xs_pad)
        #     print("xs_pad_nan_test_r:", torch.isnan(xs_pad[0].real).any())
        #     print("xs_pad_nan_test_i:", torch.isnan(xs_pad[0].imag).any())
        #     print("xs_pad_inf_test_r:", torch.isinf(xs_pad[0].real).any())
        #     print("xs_pad_inf_test_i:", torch.isinf(xs_pad[0].imag).any())
            
        #     #if torch.isnan(hs_pad).any(): # (Has printed the result of hs_pad above and thus comment this line and the line below)
        #     #    print("HS_PAD in e2e_asr_transformer_frontend.py WRONG:", hs_pad)
        #     print("hs_pad_nan_test:(after transformation)", torch.isnan(hs_pad).any())
        #     print("hs_pad_nan_all:(after transformation)", torch.isnan(hs_pad).all())
        #     #print("hs_pad_inf_test:", torch.isinf(hs_pad).any())

        # # GUO if specaug
        # kwargs = {'process': [{'type': 'time_warp', 'max_time_warp': 5, 'inplace': True, 'mode': 'PIL'}, {'type': 'freq_mask', 'F': 30, 'n_mask': 2, 'inplace': True, 'replace_with_zero': False}, {'type': 'time_mask', 'T': 40, 'n_mask': 2, 'inplace': True, 'replace_with_zero': False}]}
        # transform = Transformation(kwargs)
        # hs = transform(hs_pad)

        # 1. forward encoder
        # if hs_pad.device.index==3:
        #     print(f"hlens {hlens}, max {max(hlens)}")
        # if hs_pad.device.index==3:
        #     print(f"shape of hs before is {hs_pad.shape}")
        hs_pad = hs_pad[:, : max(hlens)]  # for data parallel
        src_mask = make_non_pad_mask(hlens.tolist()).to(hs_pad.device).unsqueeze(-2)
        # if hs_pad.device.index==3:
        #     print(f"src_mask: {src_mask.shape}")
        # if getattr(self, "mask_process", False):
        #     src_mask = self.mask_process(src_mask)
        # if hs_pad.device.index==3:
        #     print(f"src_mask: {src_mask.shape}")
        # if hs_pad.device.index==3:
        #     print(f"shape of hs before embed is {hs_pad.shape}")
        # loss_ctc = self.ctc(hs_pad.view(batch_size, -1, self.adim), hs_len, ys_pad)
        if getattr(self, "frontend_ctc", False):
            frontend_loss = self.frontend_ctc(hs_pad.view(hs_pad.size(0), -1, self.idim), hlens, ys_pad)
        ...
        self.hs_pad = hs_pad
        # if hs_pad.device.index == 0:
        #     print("hs_pad after encoder nan test:", torch.isnan(hs_pad).any())
        #     if torch.isnan(hs_pad).any():
        #         print("hs_pad after encoder nan test: WRONG", hs_pad)

        # if ys_pad.device.index == 0:
        #     print("ys_pad nan test:", torch.isnan(ys_pad).any())
        #     if torch.isnan(ys_pad).any():
        #         print("ys_pad after encoder nan test: WRONG", ys_pad)
        ...
        if self.decoder is not None:
            if self.decoder_mode == "maskctc":
                ...
                # if ys_in_pad.device.index == 0:
                #     print("maskctc ys_in/out_pad nan_test:", torch.isnan(ys_in_pad).any(), torch.isnan(ys_out_pad).any())
                #     if torch.isnan(ys_in_pad).any():
                #         print("maskctc ys_in_pad nan_test: WRONG", ys_in_pad)
                #     if torch.isnan(ys_out_pad).any():
                #         print("maskctc ys_out_pad nan_test: WRONG", ys_out_pad)
                #     print("maskctc ys_mask nan test:", torch.isnan(ys_mask).any())
                #     if torch.isnan(ys_mask).any():
                #         print("maskctc ys_mask nan test: WRONG", ys_mask)
            else:
                ...
                # if ys_in_pad.device.index == 0:
                #     print("maskctc not ys_in/out_pad nan_test:", torch.isnan(ys_in_pad).any(), torch.isnan(ys_out_pad).any())
                #     if torch.isnan(ys_in_pad).any(): 
                #         print("maskctc not ys_in_pad nan_test: WRONG", ys_in_pad)
                #     if torch.isnan(ys_out_pad).any():
                #         print("maskctc not ys_out_pad nan_test: WRONG", ys_out_pad)
                #     print("maskctc not ys_mask nan test:", torch.isnan(ys_mask).any())
                #     if torch.isnan(ys_mask).any(): 
                #         print("maskctc not ys_mask nan test: WRONG", ys_mask)

            pred_pad, pred_mask = self.decoder(ys_in_pad, ys_mask, hs_pad, hs_mask)
            self.pred_pad = pred_pad
            # if pred_pad.device.index == 0:
            #     print("pred_pad (out of decoder) nan test:", torch.isnan(pred_pad).any())
            #     if torch.isnan(pred_pad).any():
            #         print("pred_pad after encoder nan test: WRONG", pred_pad)
            #     print("pred_mask (out of decoder) nan test:", torch.isnan(pred_mask).any())
            #     if torch.isnan(pred_mask).any():
            #         print("pred_mask after encoder nan test: WRONG", pred_mask)
            ...
            # if loss_att.device.index == 0:
            #     print("loss_att nan test:", torch.isnan(loss_att).any())
            #     if torch.isnan(loss_att).any():
            #         print("loss_att nan test: WRONG", loss_att)
        ...
        if self.mtlalpha == 0.0:
            ...
            loss_ctc = self.ctc(hs_pad.view(batch_size, -1, self.adim), hs_len, ys_pad)
            # if loss_ctc.device.index == 0:
            #     print("loss_ctc nan test:", torch.isnan(loss_ctc).any())
            #     if torch.isnan(loss_ctc).any():
            #         print("loss_ctc nan test: WRONG", loss_ctc)
        ...
        if self.use_complex_beamformer:
            if isinstance(frontend_loss, torch.Tensor):
                ...
                # self.loss = self.loss + self.complex_loss_ratio * frontend_loss
            elif isinstance(loss_clean, torch.Tensor):
                self.loss = self.loss + self.complex_loss_ratio * loss_clean

        # if  self.loss.device.index == 0:
        #     print("loss_total test:", torch.isnan(self.loss).any())
        #     if torch.isnan(self.loss).any():
        #         print("loss_total nan test: WRONG", self.loss)
        return self.loss
    ...
    def encode(self, x):
        ...
        if not self.use_complex_beamformer:
            ...
            x = x.contiguous().view(1, -1, 4, 257).transpose(1,2)
            ilens = [x.shape[2]]
        
        # x = torch.as_tensor(x).unsqueeze(0)  # GUO
