...
class Universal_Frontend_Time_Attention(nn.Module):
    '''
        A univeral frontend,
    ...
    def __init__(
        ...
        if use_sub_sampling:
            ...
            if sub_sampling_type=="conv2d":
                self.embed = c_nn.ComplexConv2dSubsampling(fft_feat, fft_feat, dropout_rate)
            elif sub_sampling_type=="conv2d6":
                self.embed = c_nn.ComplexConv2dSubsampling6(fft_feat, fft_feat, dropout_rate)
            elif sub_sampling_type=="conv2d8":
                self.embed = c_nn.ComplexConv2dSubsampling8(fft_feat, fft_feat, dropout_rate)
    ...
    def forward(self, data, ilens:torch.Tensor):
        ...
        if hasattr(self, "downconv"):
            if getattr(self, "downconv_type", False) == "stack_linear":
               # B*C C' T F -> B*C 1 T C'*F
               x_r, x_i = x_r.transpose(1, 2).contiguous().view(B*C, 1, T, -1), x_i.transpose(1,2).contiguous().view(B*C, 1, T, -1)
            
            x_r, x_i = self.downconv(x_r, x_i)
        
        # B*C 1 T F -> B*C T F
        if hasattr(self, "embed"):
            x_r, x_i, ilens = self.embed(x_r, x_i, ilens)
...
class ChannelWiseAttentionModule(nn.Module):
    def __init__(
        ...
        self.pos_emd = c_nn.ComplexPositionalEncoding2D(att_feat, dropout_rate)
    ...
    def forward(self, x_r, x_i):
        ...
        out_r, out_i = self.pos_emd(out_r, out_i)
        ...
        if self.reduce_method == "mask":
            out_r, out_i = out_r*x_r + out_i*x_i, out_r*x_i - out_i*x_r  # conjung multiply
...
class ChannelWiseAttentionLayer(nn.Module):
    ...
    def forward(self, x_r, x_i):
        ...
        x_r, x_i = self.relu1(x_r + residual_r, x_i + residual_i)
        ...
        x_r, x_i = self.relu2(x_r + residual_r, x_i + residual_i)
        ...
        if self.use_residual:
            x_r, x_i = self.relu3(x_r + residual_r, x_i + residual_i)
