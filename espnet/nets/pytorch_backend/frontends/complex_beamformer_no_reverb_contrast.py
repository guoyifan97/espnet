...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        B, T, C, F = x.shape
        x -= x.mean(dim=[1, 2, 3])[:,None,None,None]
        x /= (x.pow(2).mean([1,2,3]).sqrt() + 1e-10)[:,None,None,None]
...
class Complex_Beamformer(nn.Module):
    ...
    def __init__(
        ...
        for i in self.conv_layer_list:
            ...
            if temp_inplane!=i:
                self.conv_layers.append(BasicBlock(temp_inplane, i, kernel_size=3, downsample=nn.Sequential(
                    nn.Conv2d(temp_inplane, i, kernel_size=1, groups=groups),
                    nn.GroupNorm(1, i),
                    ), groups=groups))
            else:
                self.conv_layers.append(BasicBlock(temp_inplane, i, kernel_size=3, groups=groups))
            temp_inplane = i

        # Pool block
        # self.pool_block = PoolBlock(temp_inplane, inplane, kernel_size=[4, 1])
        
        self.downconv = nn.Conv2d(temp_inplane, 1, kernel_size=1)
    ...
    def forward(self, data, ilens:torch.Tensor, dereverb_data:ComplexTensor=None, clean_data:ComplexTensor=None):
        ...
        # raise
        return data, ilens, loss, loss_reverb, loss_clean
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.conv1 = convnxn(inplanes, planes, kernel_size=kernel_size, stride=stride, groups=groups)
        self.bn1 = norm_layer(1, planes)
        self.conv2 = convnxn(planes, planes, kernel_size=kernel_size, stride=stride, groups=2)
    ...
    def forward(self, x):
        ...
        out = F.pad(x, (self.kernel_size-1, 0, self.kernel_size-1, 0))
        out = self.conv1(out)
        out = self.bn1(out) # actually layernorm
        out = self.relu(out)
        
        out = F.pad(out, (self.kernel_size-1, 0, self.kernel_size-1, 0))
        out = self.conv2(out)
        x = self.bn2(out)
...
class MSELoss(nn.Module):
    ...
    def forward(self, x_r, x_i, target_r, target_i, ilens, num_channel=0):
        ...
        #     if self.norm_func=="fp42":
        #         print(f"reverb mean and var is {target_r.mean()}, {target_r.var()}\n, x_r mean and var is {x_r.mean()}, {x_r.var()}")
        #     if self.norm_func=="fp32":
        #         print(f"clean mean and var is {target_r.mean()}, {target_r.var()}\n, x_r mean and var is {x_r.mean()}, {x_r.var()}")
