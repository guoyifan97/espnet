...
def get_mvdr_vector(
    ...
    if numerator.real.device.index == 0:
        print("numerator nan test", torch.tensor([torch.isnan(numerator.real).any(), torch.isnan(numerator.imag).any()]).any())
        if torch.tensor([torch.isnan(numerator.real).any(), torch.isnan(numerator.imag).any()]).any():
            print("numerator nan test: WRONG", numerator)
...
class DNN_Beamformer(torch.nn.Module):
    ...
    def forward(
        ...
        def apply_beamforming(data, ilens, psd_speech, psd_noise):
            ...
            if ws.device.index==0:
                print("ws nan test:", torch.tensor([torch.isnan(ws.real).any(), torch.isnan(ws.imag).any()]).any())
                if torch.tensor([torch.isnan(ws.real).any(), torch.isnan(ws.imag).any()]).any():
                    print("ws wrong:", ws)
