...
class FeatureTransform(torch.nn.Module):
    def __init__(
        ...
        if reduce:
            self.linear = torch.nn.Linear(n_fft//2+1, n_mels, bias=False)
    ...
    def forward(
        ...
        if hasattr(h, "real"):
            h = (h.real ** 2 + h.imag ** 2 + self.eps).log()
...
def feature_transform_for_complex(args, n_fft, reduce=False):
