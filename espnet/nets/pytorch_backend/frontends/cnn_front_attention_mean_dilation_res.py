...
class CNN_Front_Attention_Mean(nn.Module):
    ...
    '''
    def ilens_generator(self, ilens, subsample_factor, kernel_size):
        max_ilen = int(torch.max(ilens))
        max_ilen_delta = max_ilen - kernel_size + 1

        return torch.tensor([ceil(int(min(ilen, max_ilen_delta))/subsample_factor) for ilen in ilens]).to(ilens.device)
    ...
    def __init__(
        ...
        add_orig_spectrum_to_mid: int = -1,
        use_freq_embedding: bool = False,
        ):
        ...
        assert attention_method in ["power", "channel_wise_att", "seq", "mean", "max_pool"], f"Incompatible attention method: {attention_method}"
        ...
        if use_freq_embedding:
            self.freq_embedding = c_nn.ComplexPosEmbedding(num_embeddings=fft_feat, embedding_dim=inplane)

        temp_inplane = inplane
        self.conv_layers = nn.ModuleList()

        BasicBlock_ = BasicBlock_dilation_no_residual

        self.add_org_spectrum_to_mid = add_orig_spectrum_to_mid if add_orig_spectrum_to_mid > -1 else None

        self.ilens_subsample_g = list()
        for index, i in enumerate(self.conv_layer_list):
            if isinstance(i, list):
                ...
                if stride != 1:
                    ...
                    self.ilens_subsample_g.append((stride, kernel_size))
            else:
                # if temp_inplane!=i:
            ...
            if self.add_org_spectrum_to_mid != None and self.add_org_spectrum_to_mid == index:
                print("ADD Original data to mid!!!")
                self.add_res_affine = c_nn.ComplexConv2d(inplane, temp_inplane, kernel_size=1)
    ...
    def forward(self, data, ilens:torch.Tensor):
        ...
        for (subsample_f, k_s) in self.ilens_subsample_g:
            ilens = self.ilens_generator(ilens, subsample_f, k_s)
        ...
        #     del data
        if hasattr(self, "freq_embedding"):
            x_time_r, x_time_i = self.freq_embedding(x_time_r, x_time_i)
        else:
            del data
        ...
        for index, conv in enumerate(self.conv_layers):
            ...
            if self.add_org_spectrum_to_mid != None and index == self.add_org_spectrum_to_mid:
                res_data_r, res_data_i = self.add_res_affine(data.real, data.imag)
                x_time_r, x_time_i = x_time_r + res_data_r, x_time_i + res_data_i
...
class BasicBlock_dilation_no_residual(nn.Module):
    '''
        using n*n kernel, padding n-1 zeros in T and F
    '''
    expansion = 1
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.conv = convnxn(inplanes, planes, kernel_size=kernel_size, stride=stride, dilation=(1,dilation))
    ...
    def forward(self, x_r, x_i):
        ...
        out = c_F.complex_zero_pad(x_r, x_i, (self.dilation*(self.kernel_size_channel-1)//2, self.dilation*(self.kernel_size_channel-1)//2, self.kernel_size_time-1, 0))
        del x_r, x_i
        out = self.conv(*out)
        out = self.bn(*out) # actually layernorm
        out = self.relu(*out)
...
class MaxPoolModule(nn.Module):
    ...
    def forward(self, x_r, x_i, x_mask=None):
        return x_r.max(self.dim)[0], x_i.max(self.dim)[0], x_mask
