...
class ComplexMultiHeadedAttention(nn.Module):
    ...
    def forward_attention(self, value, scores, mask):
        ...
        if mask is not None:
            ...
            scores_r, scores_i = self.softmax(*scores)  # (batch, head, channel, time1, time2) or (batch, head, time, channel1, channel2)
            del scores
    ...
    def return_score_mat(self, query, key, value, mask=None):
        # B head time channel channel
        q, k, v = self.forward_qkv(query, key, value)
        scores = self.conjugate_transpose_matmul_conjugate(q, k)  # (batch, head, time, channel, channel)

        out = torch.sqrt(scores[0].pow(2) + scores[1].pow(2))
        # B h t c c -> B c c
        out = out.mean(dim=[1,2])
        # sysmeticalize
        # out = 0.5 * (out + out.transpose(-1, -2))

        return out, [scores[0].mean(dim=[1,2]), scores[1].mean(dim=[1,2])]
