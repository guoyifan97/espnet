...
class MIMO_Frontend_Multi_Mask(nn.Module):
    '''
...
class SpeakerSeperateModule(nn.Module):
    def __init__(self, num_spk, noise_dim=True, reduce_method="softmax", sc_assign_labels="discretize"):
        ...
        assert reduce_method in ["mean", "softmax"]
        self.reduce_method = reduce_method
        self.min_value = None
        self.softmax = c_nn.ComplexSoftmax(dim=-1)
    ...
    def generate_reduce_weights(self, labels, scores):
        ...
        if self.reduce_method == "mean":
            ...
            for cluster_id in range(self.num_clusters):
                ...
                # B C C -> B C
                temp_scores = [score.masked_fill(non_bimask.unsqueeze(2), 0.).mean(dim=1) for score in scores]
                ...
                weights[0].append(temp_scores[0].masked_fill(non_bimask, 0.)) # real
                weights[1].append(temp_scores[1].masked_fill(non_bimask, 0.)) # imag
