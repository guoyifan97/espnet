...
class Universal_Frontend(nn.Module):
    '''
        A univeral frontend,
...
class ChannelWiseAttentionModule(nn.Module):
    def __init__(
        ...
        self.att_module.append(ChannelWiseAttentionLayer(n_head=n_head, att_in_feat=att_feat, att_out_feat=in_feat, forward_layer=forward_layer, dropout_rate=dropout_rate))
    ...
    def forward(self, x_r, x_i):
        ...
        if self.reduce_method == "mask":
            out_r, out_i = out_r*x_r - out_i*x_i, out_r*x_i + out_i*x_r
...
class ChannelWiseAttentionLayer(nn.Module):
    def __init__(
        ...
        self.att_module = c_nn.ComplexChannelWiseMultiHeadedAttention(n_head, att_in_feat, att_out_feat, dropout_rate)
    ...
    def forward(self, x_r, x_i):
        ...
        x_r, x_i = self.bn1(*self.att_module([x_r, x_i], [x_r, x_i], [x_r, x_i]))
        
        
        x_r, x_i = self.relu(x_r + residual_r, x_i + residual_i)
        
        residual_r, residual_i = x_r, x_i
        x_r, x_i= self.bn2(*self.forward_layer(x_r, x_i))
        x_r, x_i = self.relu(x_r + residual_r, x_i + residual_i)
