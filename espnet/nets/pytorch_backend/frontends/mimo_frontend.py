...
class MIMO_Frontend(nn.Module):
    '''
        Use CNN as the front and use power
        attention or self-attention to get 
        mono output

        Output enhanced STFT: B x num_clusters x T x F
    '''
    ...
    def __init__(
        ...
        fft_feat: int = 257,
        att_feat: int = 256,
        att_forward_feat:int = 2048,
        dropout_rate: float = 0,
        conv_layer_dilation: List[int] = 8*[1],
        reduce_method: str = "exclude_noise",
        ...
        no_residual_no_padding: bool = False,
        mimo_num_spk: int = 2,
        mimo_apply_spk_sep: bool = False,
        mimo_cluster_reduce_method: str = "softmax",
        mimo_cluster_noise_dim : bool = True,
        mimo_cluster_assign_labels: str = "discretize",
        mimo_gaussian_norm_before:bool = False,
        ):
        ...
        assert attention_method in ["power", "channel_wise_att", "seq", "mean", "max_pool", "1d_conv_max_pool", "1d_conv_mean"], f"Incompatible attention method: {attention_method}"

        self.mimo_num_spk = mimo_num_spk
        ...
        out_channel_feat = fft_feat
        if no_residual_no_padding:
            BasicBlock_ = BasicBlock_no_residual_no_padding
            fft_feat -= len(conv_layer_list) * 4
        elif no_residual:
            BasicBlock_ = BasicBlock_no_residual
        else:
        ...
        for index, i in enumerate(self.conv_layer_list):
            if isinstance(i, list):
                ...
                if temp_inplane!=channel:
                    self.conv_layers.append(BasicBlock_(temp_inplane, channel, kernel_size=kernel_size, stride=stride, downsample=c_nn.ComplexSequential(
                        c_nn.ComplexConv2d(temp_inplane, channel, kernel_size=1),
                        c_nn.ComplexLayerNorm(channel),
                        ),
                        dilation=conv_layer_dilation[index],
                        ))
                else:
                    self.conv_layers.append(BasicBlock_(temp_inplane, channel, kernel_size=kernel_size, stride=stride, dilation=conv_layer_dilation[index],))
                temp_inplane = channel
            else:
                if temp_inplane!=i:
                    self.conv_layers.append(BasicBlock_(temp_inplane, i, kernel_size=3, downsample=c_nn.ComplexSequential(
                    ...
                    self.conv_layers.append(BasicBlock_(temp_inplane, i, kernel_size=3, dilation=conv_layer_dilation[index]))
                temp_inplane = i
        ...
        if self.attention_method == "channel_wise_att":
            ...
            self.beamforming_module = BeamformingModule(
                ...
                out_channel_feat=out_channel_feat,
                att_feat=att_feat,
                att_forward_feat=att_forward_feat,
            ...
            assert mimo_cluster_reduce_method in ["mean", "softmax", "CosSim_mean", "CosSim_softmax", "RBF_mean", "RBF_softmax"]
            print("Warning: When using 'seq' attention_method, the setting of reduce_method will be invalid")
            if "_" in mimo_cluster_reduce_method:
                self.beamforming_module = c_nn.ComplexSequential(
                    ...
                    CalSimWithKFModule(
                        num_spk=mimo_num_spk, 
                        cluster_noise_dim=mimo_cluster_noise_dim, 
                        cluster_reduce_method=mimo_cluster_reduce_method,
                        cluster_assign_labels=mimo_cluster_assign_labels,
                        dropout_rate=dropout_rate,
                        att_out_feat=fft_feat,
                        att_forward_feat=att_forward_feat,
                        gamma=1.0 / self.conv_layer_list[-1] if isinstance(self.conv_layer_list[-index], int) else 1.0 / self.conv_layer_list[-1][0],
                        gaussian_norm_before=mimo_gaussian_norm_before,
                    ),
                    PowerAttentionModule(
                        window_len=power_att_window_len,
                        affine=power_att_affine,
                        fft_feat=out_channel_feat,
                        softmax_factor=power_att_softmax_factor,
                        auto_grad_softmax_factor=power_att_auto_grad_softmax_factor,
                        use_delta_power_att=power_att_use_delta_power_att, 
                        auto_grad_delta_factor=power_att_auto_grad_delta_factor, 
                ...
                self.beamforming_module = c_nn.ComplexSequential(
                    BeamformingModule(
                        ...
                        return_mask=True,
                        use_pos_embed_in_beamform_layer=use_pos_embed_in_beamform_layer,
                        apply_spk_sep=mimo_apply_spk_sep,
                        cluster_noise_dim=mimo_cluster_noise_dim,
                        num_spk=mimo_num_spk,
                        cluster_reduce_method=mimo_cluster_reduce_method,
                        cluster_assign_labels=mimo_cluster_assign_labels,
        ...
        elif self.attention_method == "max_pool":
            self.beamforming_module = c_nn.ComplexSequential(
                    BeamformingModule(
                        n_blocks=n_att_blocks,
                        n_channel_blocks=n_channel_att_blocks,
        ...
        elif self.attention_method == "1d_conv_max_pool":
            self.beamforming_module = c_nn.ComplexSequential(
                    BeamformingModule(
                        n_blocks=n_att_blocks,
                        n_channel_blocks=n_channel_att_blocks,
                    ...
                    MaxPoolModule(dim=1, cnn_d=True, inplane=temp_inplane),
            )
        elif self.attention_method == "1d_conv_mean":
            self.beamforming_module = c_nn.ComplexSequential(
                    BeamformingModule(
                        n_blocks=n_att_blocks,
                        n_channel_blocks=n_channel_att_blocks,
                    ...
                    MeanModule(dim=1, cnn_d=True, inplane=temp_inplane),
            )
    ...
    def forward(self, data, ilens:torch.Tensor):
        ...
        # CASES using frontend as separator GUO
        if x_time_r.dim() == 4:
            # B num_spk T F -> num_spk*B T F
            x_time_r = x_time_r.transpose(0,1).contiguous().view(-1, T, F)
            x_time_i = x_time_i.transpose(0,1).contiguous().view(-1, T, F)
            # B -> num_spk*B
            ilens = torch.cat([ilens for _ in range(self.mimo_num_spk)], dim=0)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        # BCTF
        self.conv1 = convnxn(inplanes, planes, kernel_size=kernel_size, stride=(1, stride0), dilation=(1,dilation[0]))
        self.bn1 = norm_layer(planes)
        self.conv2 = convnxn(planes, planes, kernel_size=kernel_size, stride=(1, stride1), dilation=(1, dilation[1]))
    ...
    def forward(self, x_r, x_i):
        ...
        out = c_F.complex_zero_pad(x_r, x_i, (self.dilation[0]*(self.kernel_size_channel-1)//2, self.dilation[0]*(self.kernel_size_channel-1)//2, self.kernel_size_time-1, 0))
        del x_r, x_i
        out = self.conv1(*out)
        out = self.bn1(*out) # actually layernorm
        out = self.relu(*out)
        
        out = c_F.complex_zero_pad(out[0], out[1], (self.dilation[1]*(self.kernel_size_channel-1)//2, self.dilation[1]*(self.kernel_size_channel-1)//2, self.kernel_size_time-1, 0))
        out = self.conv2(*out)
...
class BasicBlock_no_residual(nn.Module):
    '''
        using n*n kernel, padding n-1 zeros in T and F
    '''
    expansion = 1
...
class BasicBlock_no_residual_no_padding(nn.Module):
    '''
        using n*n kernel, padding n-1 zeros in T and F
    '''
    expansion = 1
    ...
    def forward(self, x_r, x_i):
        ...
        # identity_r, identity_i = x_r, x_i
        out = c_F.complex_zero_pad(x_r, x_i, (0, 0, self.kernel_size_time-1, 0))
        del x_r, x_i
        out = self.conv1(*out)
        out = self.bn1(*out) # actually layernorm
        out = self.relu(*out)
        
        out = c_F.complex_zero_pad(out[0], out[1], (0, 0, self.kernel_size_time-1, 0))
        out = self.conv2(*out)
        x_r, x_i = self.bn2(*out)
...
class BeamformingModule(nn.Module):
    def __init__(
        ...
        apply_spk_sep: bool = False,
        num_spk: int = 2,
        cluster_reduce_method: str = "softmax",
        cluster_noise_dim : bool = True,
        cluster_assign_labels: str = "discretize",
        ):
        ...
        if in_channel_feat != att_feat:
            ...
            self.after_att_ln = c_nn.ComplexLinear(att_feat, out_channel_feat)
            ...
            if n_channel_blocks > 0:
                self.att_module.append(
                    BeamformingLayerShengDai(
                        ...
                        use_pos_embed_in_beamform_layer=use_pos_embed_in_beamform_layer,
                        pos_embed_type=pos_embed_type,
                        apply_spk_sep=apply_spk_sep,
                        cluster_noise_dim=cluster_noise_dim,
                        num_spk=num_spk,
                        cluster_reduce_method=cluster_reduce_method,
                        cluster_assign_labels=cluster_assign_labels,
                    )
                )
        ...
        elif module_type == "shengdai_time":
            self.att_module = nn.ModuleList()
            for _ in range(n_channel_blocks-1):
                self.att_module.append(
                    BeamformingLayerShengDai(
...
class BeamformingLayerAoLiAo(nn.Module):
    def __init__(
        ...
        self.forward_layer = c_nn.ComplexSequential(
            c_nn.ComplexLinear(att_out_feat, att_forward_feat),
            c_nn.ComplexLeakyReLU(inplace=True),
            c_nn.ComplexDropout(dropout_rate),
            c_nn.ComplexLinear(att_forward_feat, att_out_feat),
            )
...
class BeamformingLayerShengDai(nn.Module):
    def __init__(
        ...
        if apply_spk_sep:
            self.ssm = SpeakerSeperateModule(
                num_spk, 
                noise_dim=cluster_noise_dim, reduce_method=cluster_reduce_method,
                sc_assign_labels=cluster_assign_labels,)
        ...
        self.apply_spk_sep = apply_spk_sep
    ...
    def forward(self, x_channel_r, x_channel_i, x_channel_mask):
        ...
        if self.ssm != None:
            # Speaker Seperate Module
            # B C C      [B C C]*2
            score_power, score = self.att_module.return_score_mat([x_channel_r, x_channel_i], [x_channel_r, x_channel_i], [x_channel_r, x_channel_i], x_channel_mask)

            # B C T F -> B num_clusters T F
            x_channel_r, x_channel_i = self.dropout(*self.ssm(x_channel_r, x_channel_i, score_power, score))

        else:
            if hasattr(self, "pos_embed"):
                assert self.apply_spk_sep == False
                x_channel_r_pos_enc, x_channel_i_pos_enc = self.pos_embed(x_channel_r, x_channel_i)
                x_channel_r, x_channel_i = self.dropout(*self.att_module(
                    [x_channel_r_pos_enc, x_channel_i_pos_enc], 
                    [x_channel_r_pos_enc, x_channel_i_pos_enc], 
...
class CalSimWithKFModule(nn.Module):
    def __init__(
        self, 
        num_spk, 
        cluster_noise_dim, 
        cluster_reduce_method, 
        cluster_assign_labels, 
        dropout_rate, 
        att_out_feat,
        att_forward_feat,
        gamma=1.0,
        gaussian_norm_before=False,
        ):
        super().__init__()
        self.norm1 = c_nn.ComplexLayerNorm(1, affine=False)
        self.norm2 = c_nn.ComplexLayerNorm(1, affine=False)
        self.dropout = c_nn.ComplexDropout(dropout_rate)
        # assert cluster_reduce_method == "mean"
        self.gaussian_norm_before = gaussian_norm_before 
        self.ssm = SpeakerSeperateModule(
            num_spk, 
            noise_dim=cluster_noise_dim, 
            reduce_method=cluster_reduce_method,
            sc_assign_labels=cluster_assign_labels,)
        self.num_spk = num_spk
        self.forward_layer = c_nn.ComplexSequential(
            c_nn.ComplexLinear(att_out_feat, att_forward_feat),
        ...
        kernel_fn = cluster_reduce_method.split("_")[0]
        assert kernel_fn in ["CosSim", "RBF"]
        self.k_f = {
            "CosSim": self.cos_sim_fn,
            "RBF": self.guassian_euclidean_fn,
            }[kernel_fn]
        if kernel_fn == "RBF":
            # self.gamma = torch.nn.Parameter(torch.tensor(gamma))
            self.gamma = torch.tensor(gamma)

    def guassian_euclidean_fn(self, x_p):
        """
            :torch.Tensor x_p: B C T F

            :return torch.Tensor: B T C C
        """
        def euclidean_dist(x):
            # x: B*T C F
            B, C, F = x.shape
            # C C C
            if not hasattr(self, "cal_mat"):
                cal_mat = torch.stack([-1*torch.eye(C) for _ in range(C)], dim=0).to(x.device)
                for i in range(C):
                    cal_mat[i][:, i] += torch.ones(C).to(x.device)
                self.cal_mat = cal_mat
            
            # B*T C F -> B*T C C
            # 这里省掉一次开根号，和高斯分布的平方消去了
            score = torch.einsum("lmc,bcf->blmf", self.cal_mat, x).pow(2).sum(dim=-1)
            return score
        
        def euclidean_dist_v2(x):
            # Memory Comsumption Mode
            # x: B*T C F
            B, C, F = x.shape
            # B*T C C
            score = torch.zeros(B, C, C).to(x.device)
            for row in range(C):
                for col in range(row):
                    # B*T F -> B*T
                    score[:, row, col] = (x[:, row] - x[:, col]).pow(2).sum(dim=-1)
        ...
        def guassian_norm(x):
            # x: B T C C
            return torch.exp(-self.gamma * x)

        B, C, T, F = x_p.shape
        if self.gaussian_norm_before:
            x_p = x_p / x_p.sum(dim=-1, keepdim=True)

        #B C T F -> B T C F -> B*T C F
        x_p = x_p.transpose(1, 2).contiguous().view(-1, C, F)

        # B*T C F -> B*T C C -> B T C C
        try:
            score = euclidean_dist(x_p).view(B, T, C, C)
        except:
            # memory comsumption mode
            score = euclidean_dist_v2(x_p).view(B, T, C, C)

        # B T C C -> B T C C
        score = guassian_norm(score).detach()

        return score 

    def cos_sim_fn(self, x_p):
        """
            :torch.Tensor x_p: B C T F
        ...
        B, C, T, F = x_p.shape
        # B C T F
        x_p_norm = x_p / x_p.sum(dim=-1, keepdim=True)
        
        # B C T F -> BxT C F
        x_p_norm_l = x_p_norm.transpose(1, 2).contiguous().view(-1, C, F)
        # BxT C F -> BxT F C
        x_p_norm_r = x_p_norm_l.transpose(1, 2)

        # BxT C C -> B T C C
        score = torch.matmul(x_p_norm_l, x_p_norm_r).view(B,T,C,C).detach()

        return score

    def forward(self, x_channel_r, x_channel_i, x_channel_mask):
        B, C, T, F = x_channel_r.shape
        x_channel_r, x_channel_i = self.norm1(x_channel_r, x_channel_i)
        # B C T F
        x_p = x_channel_r**2 + x_channel_i**2

        # B T T  -> B T -> B T 1 1
        mask = x_channel_mask[..., 0].unsqueeze(-1).unsqueeze(-1).eq(0)
        # B T T -> B -> B 1 1
        ilens = x_channel_mask[..., 0].sum().unsqueeze(-1).unsqueeze(-1)
        # B C T F -> B T C C -> B C C
        score = self.k_f(x_p).masked_fill(mask, 0.).sum(dim=1) / ilens
        
        x_channel_r, x_channel_i = self.dropout(*self.ssm(x_channel_r, x_channel_i, score, score))
...
class PowerAttentionModule(nn.Module):
    ...
    def __init__(self, 
        ...
        assert reduce_method in ["max", "softmax", "channel_att_max", "double_score", "channel_att_max_v2", "double_score_v2", "double_score_v3", "exclude_noise"], f'the reduce method of power_att module must be in \
            ["max", "softmax", "channel_att_max", "double_score","channel_att_max_v2", "double_score_v2", "double_score_v3", "exclude_noise"]'
    ...
    def _delta_power_forward(self, x_r, x_i, mask=None):
        ...
        if self.reduce_method == "exclude_noise":
            # BC -> BC -> BC11
            min_mask = torch.eq(power_att, torch.min(power_att, dim=1, keepdim=True)[0]).unsqueeze(-1).unsqueeze(-1)
            exclude_noise_mask = ~min_mask
            return x_r.masked_select(exclude_noise_mask).view(B, -1, T, F), x_i.masked_select(exclude_noise_mask).view(B, -1, T, F), mask
...
class SpeakerSeperateModule(nn.Module):
    def __init__(self, num_spk, noise_dim=True, reduce_method="softmax", sc_assign_labels="discretize"):
        super().__init__()
        self.num_clusters = num_spk + 1 if noise_dim else num_spk
        assert sc_assign_labels in ["discretize", "kmeans"]
        self.cluster = SpectralClustering(
            n_clusters=self.num_clusters, 
            affinity="precomputed",
            assign_labels=sc_assign_labels,
        )
        assert reduce_method in ["mean", "softmax", "CosSim_mean", "CosSim_softmax", "RBF_mean", "RBF_softmax"]
        self.reduce_method = reduce_method
        self.min_value = None
        self.softmax = c_nn.ComplexSoftmax(dim=-1)

    def generate_reduce_weights(self, labels, scores):
        """
            :param torch.Tensor label: B C
            :param torch.Tensor score: [(B C C)*2]
            :return torch.Tensor weights: B num_clusters C or [(B num_clusters C)*2] (real and imag)
        """
        if self.min_value == None:
            self.min_value = float(finfo(torch.tensor(0, dtype=scores[0].dtype).numpy().dtype).min)

        if self.reduce_method.endswith("mean"):
            weights = []
            for cluster_id in range(self.num_clusters):
                # B C
                weights.append((labels == cluster_id).float().to(scores[0].device))
            ...
            if isinstance(scores, torch.Tensor):
                ...
                for cluster_id in range(self.num_clusters):
                    # B C
                    bimask = (labels == cluster_id).to(scores.device)
                    if (bimask == False).all():
                        logging.info(f"Warning: No spectral is assined to {cluster_id}")
                    non_bimask = ~bimask
                    
                    # B C C -> B C
                    temp_scores = scores.masked_fill(non_bimask.unsqueeze(2), 0.).sum(dim=1) / bimask.sum(dim=-1, keepdim=True)
                    
                    # B C
                    temp_scores = torch.nn.functional.softmax(temp_scores.masked_fill(non_bimask, self.min_value), dim=-1)

                    temp_scores = temp_scores.masked_fill(non_bimask, 0.) 

                    weights.append(temp_scores)
                    
                # [(B,C)] -> (B, num_clusters C) 
                return torch.stack(weights, dim=1)
                ...
                for cluster_id in range(self.num_clusters):
                    # B C
                    bimask = (labels == cluster_id).to(scores[0].device)
                    if (bimask == False).all():
                        logging.info(f"Warning: No spectral is assined to {cluster_id}")
                    non_bimask = ~bimask
                    
                    # B C C -> B C
                    temp_scores = [score.masked_fill(non_bimask.unsqueeze(2), 0.).sum(dim=1) / bimask.sum(dim=-1, keepdim=True) for score in scores]
                    
                    temp_scores = self.softmax(temp_scores[0], temp_scores[1], mask=non_bimask, min_value=self.min_value)
                    temp_scores = [temp_score.masked_fill(non_bimask, 0.) for temp_score in temp_scores]

                    weights[0].append(temp_scores[0]) # real
                    weights[1].append(temp_scores[1]) # imag
                # [(B,C), ...] -> [(B, num_clusters C) *2]
                return [torch.stack(weight, dim=1) for weight in weights]

    def matmul(self, m1, m2):
        B, C, T, F = m2[0].shape
        if not isinstance(m1, list):
            return torch.matmul(m1, m2[0].view(B, C, -1)).view(B, -1, T, F), torch.matmul(m1, m2[1].view(B, C, -1)).view(B, -1, T, F)
        else:
            return [torch.matmul(m1[0],m2[0].view(B, C, -1)).view(B, -1, T, F) - torch.matmul(m1[1],m2[1].view(B, C, -1)).view(B, -1, T, F), torch.matmul(m1[1],m2[0].view(B, C, -1)).view(B, -1, T, F) + torch.matmul(m1[0],m2[1].view(B, C, -1)).view(B, -1, T, F)]

    def forward(self, x_r, x_i, score_p, score):
        """
            :param torch.Tensor x_r, x_i: B C T F
            :param torch.Tensor score_p: B, C, C
            :param torch.Tensor score: [(B, C, C), (B, C, C)]
            :param 
            :return torch.Tensor: B num_clusters, T, F
            
        """
        labels = []
        for temp_score_p in score_p:
            temp_score_p_cpu = temp_score_p.detach().cpu().numpy()
            labels.append(list(self.cluster.fit_predict(0.5*(temp_score_p_cpu + temp_score_p_cpu.T))))
        # B C
        labels = torch.tensor(labels).to(score_p.device)
        # B num_clusters C
        weights = self.generate_reduce_weights(labels, score)
        # B num_clusters C MULTIPLY B C T F 

        return self.matmul(weights, [x_r, x_i])
...
class MeanModule(nn.Module):
    def __init__(self, dim=1, cnn_d=False, inplane=50):
        super().__init__()
        self.dim = dim
        if cnn_d:
            self.cnn_d = True
            self.dconv = c_nn.ComplexConv2d(inplane, inplane//3, 1)

    def forward(self, x_r, x_i, x_mask=None):
        if getattr(self, "cnn_d", False):
            x_r, x_i = self.dconv(x_r, x_i)
...
class MaxPoolModule(nn.Module):
    ...
    def forward(self, x_r, x_i, x_mask=None):
        ...
        return x_r.max(dim=self.dim)[0], x_i.max(dim=self.dim)[0], x_mask
...
if __name__ == "__main__":
    # B, C, T, F = 5, 40, 100, 32
    # SSM1 = SpeakerSeperateModule(2, reduce_method="softmax")
    # SSM2 = SpeakerSeperateModule(2, reduce_method="mean")
    # x_r, x_i = torch.rand(B,C,T,F), torch.rand(B,C,T,F)
    # score_p = torch.rand(B,C,C)
    # score = [torch.rand(B,C,C), torch.rand(B,C,C)]
    # out1 = SSM1(x_r, x_i, score_p, score)
    # out2 = SSM2(x_r, x_i, score_p, score)
    frontend = MIMO_Frontend(
        attention_method="seq",
        conv_layer_list=[4,8,16,32],
        inplane=4,
        no_residual=True,
        n_time_att_blocks=0,
        use_pos_embed=False,
        n_att_blocks=0,
        n_channel_att_blocks=2,
        num_abs_pos_extracted=2,
        power_att_affine=True,
        power_att_use_delta_power_att=True,
        downconv_type="stack_linear",
        conv_layer_dilation=[[1,1],[1,1],[1,1],[1,1]],
        mimo_apply_spk_sep=True,
    )
    print(frontend)
    B, C, T, F = 5, 4, 100, 257
    data_in = ComplexTensor(torch.rand(B,T,C,F), torch.rand(B,T,C,F))
    ilens = torch.tensor([100, 99, 98, 100, 97])
    data_out, ilens, frontend_loss = frontend(data_in, ilens)
