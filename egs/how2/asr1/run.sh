...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for x in train val; do
        clean_corpus.sh --maxframes 3000 --maxchars 400 --utt_extra_files "text.tc text.lc text.lc.rm" data/${x} "en pt"
    done
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    cut -f 2- -d' ' data/train.en/text.${src_case} | grep -o -P '&[^;]*;'| sort | uniq > ${nlsyms}
    cat ${nlsyms}

    echo "make a dictionary"
    echo "<unk> 1" > ${dict} # <unk> must be 1, 0 will be used for "blank" in CTC
    offset=$(wc -l < ${dict})
    cut -f 2- -d' ' data/train.en/text.${src_case} | grep -v -e '^\s*$' > data/lang_1spm/input_${src_case}.txt
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    mkdir -p ${lmdatadir}
    cut -f 2- -d " " data/${train_set}/text.${src_case} | spm_encode --model=${bpemodel}.model --output_format=piece \
