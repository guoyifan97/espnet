    ...
    for cond in tqdm.tqdm(conditions):
        ...
        for meet in meeting:
            ...
            wav_path = os.path.join(
                os.path.abspath(meet), "record", "raw_recording.wav"
            )
            ...
            with open(
                os.path.join(
                    os.path.abspath(meet), "transcription", "meeting_info.txt"
                ),
                "r",
            ) as f:
                next(f)
                for line in f:
                    ...
                    )  # 6930_Session0_CH1_0L_000853_002463
...
def make_argparse():
    parser = argparse.ArgumentParser(
        description="Reorganize LibriCSS data into Kaldi format."
    )
