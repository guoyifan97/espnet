...
if [ ${stage} -le -1 ] && [ ${stop_stage} -ge -1 ]; then
    ...
    echo "Please download from https://iwslt.org/2021/low-resource and put data into ${iwslt_low_resource}"
fi
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    offset=$(wc -l < ${dict})
    grep sp1.0 data/train_sp.${src_lang}-${tgt_lang}.*/text.${tgt_case} | cut -f 2- -d' ' | grep -v -e '^\s*$' > data/lang_1spm/input_${tgt_lang}_${src_case}_${tgt_case}.txt
