...
src_lang=swa
tgt_lang=en
# you can choose from swa-eng and swc-fra
...
# exp tag
tag="bpe${nbpe}" # tag for managing experiments.
...
train_dev=valid.${src_lang}-${tgt_lang}.${src_lang}
recog_set="valid_org.${src_lang}-${tgt_lang}.${src_lang}"

if [ ${stage} -le -1 ] && [ ${stop_stage} -ge -1 ]; then
    echo "stage -1: Data Download"
    echo "download from https://iwslt.org/2021/low-resource to ${iwslt_low_resource}"
fi
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    python local/data_prep.py ${iwslt_low_resource} ${src_lang} ${tgt_lang}
    for set in train valid; do
        dst=data/${set}.${src_lang}-${tgt_lang}
        ./local/data_prep.sh ${dst} ${src_lang} ${tgt_lang}
    done
    ./utils/fix_data_dir.sh --utt_extra_files "text.tc.${src_lang} text.lc.${src_lang} text.lc.rm.${src_lang} text.tc.${tgt_lang} text.lc.${tgt_lang} text.lc.rm.${tgt_lang}" data/valid.${src_lang}-${tgt_lang}
    ./utils/fix_data_dir.sh --utt_extra_files "text.tc.${src_lang} text.lc.${src_lang} text.lc.rm.${src_lang} text.tc.${tgt_lang} text.lc.${tgt_lang} text.lc.rm.${tgt_lang}" data/train.${src_lang}-${tgt_lang}
fi
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for lang in $(echo ${tgt_lang} | tr '_' ' '); do
        for x in valid.${src_lang}-${tgt_lang}; do
    ...
    # speed perturbation
    speed_perturb.sh --cmd "$train_cmd" --cases "lc.rm lc tc" --langs "${src_lang} ${tgt_lang}" data/train.${src_lang}-${tgt_lang} data/train_sp.${src_lang}-${tgt_lang} ${fbankdir}
    ...
    for x in train_sp.${src_lang}-${tgt_lang} valid.${src_lang}-${tgt_lang}; do
        clean_corpus.sh --maxframes 3000 --maxchars 500 --utt_extra_files "text.tc text.lc text.lc.rm" data/${x} "${src_lang} ${tgt_lang}"
    done
