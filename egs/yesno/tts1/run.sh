...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    # make a dev set
    utils/subset_data_dir.sh --last data/train_yesno 10 data/deveval
    utils/subset_data_dir.sh --last data/deveval 5 data/${eval_set}
    utils/subset_data_dir.sh --first data/deveval 5 data/${train_dev}
    n=$(( $(wc -l < data/train_yesno/wav.scp) - 5))
    utils/subset_data_dir.sh --first data/train_yesno  ${n} data/${train_set}
