...
if [ ! -d links/LDC2010S01 -o ! -d links/LDC2010T04 ]; then
    echo "The speech and the data directories need to be named LDC2010S01 and LDC2010T04 respectively"
    exit 1;
fi

#if [ ! -d links/LDC2010S01/DISC1/data/speech -o ! -d links/LDC2010S01/DISC2/data/speech ];
if [ ! -d links/LDC2010S01/data/speech ]; then
...
if [ ! -d links/LDC2010T04/fisher_spa_tr/data/transcripts ]; then
...
if [ $fcount_s != 819 -o $fcount_t != 819 ]; then
...
if [ $stage -le 2 ]; then
    ...
    # NOTE: do not remove here (remove later in normalize_trans.sh)
